...
POLICY_TEMPLATES: dict[PolicyType, dict] = {
    ...
    PolicyType.VENDOR_MANAGEMENT: {
        "title": "AI調達・ベンダー管理方針",
        "description": "AIシステム・サービスの調達およびベンダー管理に関する方針",
        "sections": [
            {
                "heading": "1. 目的",
                "content": (
                    "本方針は、{org_name}がAIシステム・サービスを外部から調達する際の"
                    "評価基準、契約条件、継続的管理の方針を定める。"
                ),
            },
            {
                "heading": "2. 適用範囲",
                "content": (
                    "以下を含む全てのAI関連調達に適用する：\n"
                    "- AI-as-a-Service（API）の利用契約\n"
                    "- AIモデル・アルゴリズムの購入・ライセンス\n"
                    "- AI開発の外部委託\n"
                    "- 学習データの購入・利用許諾"
                ),
            },
            {
                "heading": "3. ベンダー評価基準",
                "content": (
                    "3.1 技術的信頼性: モデルの精度・安定性・スケーラビリティ\n"
                    "3.2 セキュリティ: データ保護、暗号化、アクセス制御\n"
                    "3.3 透明性: モデルの説明可能性、ドキュメンテーション\n"
                    "3.4 倫理的配慮: バイアス軽減、公平性への取り組み\n"
                    "3.5 コンプライアンス: 関連法規・ガイドラインへの準拠\n"
                    "3.6 事業継続性: ベンダーの財務健全性、事業継続計画"
                ),
            },
            {
                "heading": "4. 契約要件",
                "content": (
                    "4.1 データ取扱条件: データの所有権、利用範囲、削除義務\n"
                    "4.2 品質保証: SLA（精度、応答時間、可用性）\n"
                    "4.3 監査権限: 当社による監査・検査の権限\n"
                    "4.4 責任分界: AIの判断に起因する損害の責任分担\n"
                    "4.5 解約条件: サービス終了時のデータ返却・削除"
                ),
            },
            {
                "heading": "5. 継続的管理",
                "content": (
                    "5.1 定期評価: 年1回以上のベンダー評価を実施する\n"
                    "5.2 性能モニタリング: SLA達成状況を継続的に監視する\n"
                    "5.3 変更管理: ベンダー側のモデル変更・アップデートを追跡する"
                ),
            },
        ],
    },

    PolicyType.MODEL_MANAGEMENT: {
        "title": "AIモデル管理方針",
        "description": "AIモデルのライフサイクル管理に関する方針",
        "sections": [
            {
                "heading": "1. 目的",
                "content": (
                    "本方針は、{org_name}が開発・利用するAIモデルのライフサイクル全体にわたる"
                    "管理基準を定め、モデルの品質と信頼性を確保することを目的とする。"
                ),
            },
            {
                "heading": "2. モデルインベントリ",
                "content": (
                    "全AIモデルを台帳（インベントリ）で管理する。記録項目：\n"
                    "- モデル名・バージョン・用途・リスクレベル\n"
                    "- 責任者（オーナー）・開発者\n"
                    "- 使用データ・学習条件・性能指標\n"
                    "- デプロイ先・利用開始日・ステータス"
                ),
            },
            {
                "heading": "3. モデル開発プロセス",
                "content": (
                    "3.1 要件定義: ビジネス要件と技術要件の明確化\n"
                    "3.2 データ準備: データ品質基準に基づく前処理\n"
                    "3.3 モデル設計・学習: 再現性を確保した実験管理\n"
                    "3.4 検証・テスト: 精度、公平性、堅牢性のテスト\n"
                    "3.5 レビュー: 独立したモデルレビューの実施\n"
                    "3.6 承認: リスクレベルに応じた承認プロセス"
                ),
            },
            {
                "heading": "4. デプロイメント管理",
            ...
            },
            {
                "heading": "5. 運用モニタリング",
                "content": (
                    "5.1 性能指標のモニタリング（精度、応答時間等）\n"
                    "5.2 データドリフト・概念ドリフトの検出\n"
                    "5.3 アラート閾値の設定と異常時の対応手順\n"
                    "5.4 定期的な再学習・再評価のスケジュール"
                ),
            },
            {
                "heading": "6. モデル退役",
                "content": (
                    "6.1 退役基準の定義（性能劣化、後継モデルへの移行等）\n"
                    "6.2 退役プロセス（利害関係者への通知、データ・モデルの保管/廃棄）\n"
                    "6.3 退役後の記録保持"
                ),
            },
        ],
    ...
    PolicyType.TRANSPARENCY: {
        "title": "AI透明性方針",
        "description": "AIの説明可能性と情報開示に関する方針",
        "sections": [
            {
                "heading": "1. 目的",
                "content": (
                    "本方針は、{org_name}がAIの利用に関する透明性を確保し、"
                    "利害関係者に対して適切な情報開示と説明を行うための方針を定める。"
                ),
            },
            {
                "heading": "2. AI利用の開示",
                "content": (
                    "2.1 AIを利用していることの明示: サービス画面・利用規約等で開示する\n"
                    "2.2 AI生成コンテンツの明示: AIが生成したコンテンツにはその旨を表示する\n"
                    "2.3 開示の対象: 顧客、従業員、取引先等の利害関係者"
                ),
            },
            {
                "heading": "3. 判断根拠の説明",
                "content": (
                    "3.1 説明のレベル: 利害関係者の理解度に合わせた説明を提供する\n"
                    "3.2 技術的説明: モデルカード、データシートの公開\n"
                    "3.3 利用者向け説明: AI判断の主な要因を平易な言葉で説明する\n"
                    "3.4 XAI技術: 必要に応じて説明可能AI技術（SHAP、LIME等）を適用する"
                ),
            },
            {
                "heading": "4. 情報開示の範囲",
                "content": (
                    "4.1 公開情報: AI利用方針、倫理原則、ガバナンス体制\n"
                    "4.2 要請に応じた情報: AIの判断根拠、使用データの種類\n"
                    "4.3 非開示情報: 営業秘密、セキュリティ上の理由で非開示とするもの\n"
                    "4.4 非開示の場合はその理由を説明する"
                ),
            },
            {
                "heading": "5. フィードバック",
                "content": (
                    "5.1 AI判断に対する問い合わせ窓口を設置する\n"
                    "5.2 異議申立て手続きを整備する\n"
                    "5.3 フィードバックをサービス改善に活用する"
                ),
            },
        ],
    },
    ...
    PolicyType.FAIRNESS: {
        "title": "AI公平性方針",
        "description": "AIのバイアス検出・是正および公平性確保に関する方針",
        "sections": [
            {
                "heading": "1. 目的",
                "content": (
                    "本方針は、{org_name}が開発・利用するAIシステムにおいて"
                    "不当な差別を防止し、公平性を確保するための方針を定める。"
                ),
            },
            {
                "heading": "2. 公平性の定義",
                "content": (
                    "2.1 保護属性: 性別、年齢、人種、国籍、障害、宗教等\n"
                    "2.2 公平性基準: 利用文脈に応じて適切な公平性の定義を選択する\n"
                    "  - 統計的パリティ（Demographic Parity）\n"
                    "  - 均等機会（Equalized Odds）\n"
                    "  - 個人公平性（Individual Fairness）\n"
                    "2.3 公平性基準の選択根拠を文書化する"
                ),
            },
            {
                "heading": "3. バイアス評価プロセス",
                "content": (
                    "3.1 データバイアス: 学習データの偏りを評価する\n"
                    "3.2 モデルバイアス: モデル出力の公平性を測定する\n"
                    "3.3 運用バイアス: 実運用でのバイアスを継続的にモニタリングする\n"
                    "3.4 評価頻度: 導入前、定期的（四半期推奨）、重大変更時"
                ),
            },
            {
                "heading": "4. 是正措置",
                "content": (
                    "4.1 バイアス検出時の報告・エスカレーション手順\n"
                    "4.2 技術的是正: データ再サンプリング、モデル再学習、閾値調整等\n"
                    "4.3 組織的是正: プロセス見直し、教育強化等\n"
                    "4.4 是正後の効果検証"
                ),
            },
            {
                "heading": "5. 多様性の確保",
                "content": (
                    "5.1 AI開発チームに多様なバックグラウンドの人材を確保する\n"
                    "5.2 AI評価に多様な視点を含める\n"
                    "5.3 利害関係者からのフィードバックを定期的に収集する"
                ),
            },
        ],
    },
    ...
    PolicyType.AUDIT: {
        "title": "AI監査方針",
        "description": "AIシステムの内部監査・外部監査に関する方針",
        "sections": [
            {
                "heading": "1. 目的",
                "content": (
                    "本方針は、{org_name}がAIシステムに対する監査を計画的に実施し、"
                    "ガバナンスの有効性を確認・改善するための方針を定める。"
                ),
            },
            {
                "heading": "2. 監査の種類",
                "content": (
                    "2.1 内部監査: 社内チームによる定期的な監査\n"
                    "2.2 外部監査: 第三者機関による独立した監査\n"
                    "2.3 特別監査: インシデント発生時や重大変更時の臨時監査"
                ),
            },
            {
                "heading": "3. 内部監査",
                "content": (
                    "3.1 監査計画: 年間監査計画を策定し、全AIシステムをカバーする\n"
                    "3.2 監査項目: ガバナンス体制、リスク管理、データ品質、公平性、"
                    "セキュリティ、透明性、法令遵守\n"
                    "3.3 監査員: AI/MLの知識を持つ人員を配置する\n"
                    "3.4 独立性: 監査対象と独立した監査員を確保する\n"
                    "3.5 報告: 監査結果を経営層に報告する"
                ),
            },
            {
                "heading": "4. 外部監査",
                "content": (
                    "4.1 実施条件: ISO 42001認証取得時、EU AI Act適合性評価時等\n"
                    "4.2 認定評価機関の選定基準\n"
                    "4.3 事前準備: 技術文書、記録、エビデンスの整備"
                ),
            },
            {
                "heading": "5. 是正措置と追跡",
                "content": (
                    "5.1 監査指摘事項の分類（重大/軽微/観察事項）\n"
                    "5.2 是正措置計画の策定と責任者の割り当て\n"
                    "5.3 是正措置の実施状況の追跡\n"
                    "5.4 是正措置の有効性の確認"
                ),
            },
